...
public class SpiritBoxer extends Enemy {
    ...
    public void loadAnimations() {
        ...
        loadAnimation(CardTDGame.ASSETSMANAGER.<Texture>get(StaticVariables.SPIRITBOXER_IDLE), rows, cols, EnemyState.IDLE);

        rows = 6;

        loadAnimation(CardTDGame.ASSETSMANAGER.<Texture>get(StaticVariables.SPIRITBOXER_RUN), rows, cols, EnemyState.RUN);

        rows = 10;

        loadAnimation(CardTDGame.ASSETSMANAGER.<Texture>get(StaticVariables.SPIRITBOXER_ATTACK), rows, cols, EnemyState.ATTACK);

        rows = 8;

        loadAnimation(CardTDGame.ASSETSMANAGER.<Texture>get(StaticVariables.SPIRITBOXER_DEATH), rows, cols, EnemyState.DYING);

        rows = 4;

        loadAnimation(CardTDGame.ASSETSMANAGER.<Texture>get(StaticVariables.SPIRITBOXER_DAMAGED), rows, cols, EnemyState.DAMAGED);
